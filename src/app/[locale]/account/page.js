...
import { KeyRound, Save, Trash2, RefreshCw, ExternalLink, CreditCard, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function AccountPage() {
  ...
  const credentials = useRemoteResource(status === "authenticated" ? "/api/user/credentials" : null);
  ...
  async function saveKey(event) {
    ...
    try { await api("/api/user/credentials", { method: "POST", body: { provider, secret } }); setSecret(""); credentials.reload(); } catch (err) { fail(err); } finally { setBusy(false); }
  }
  async function confirm() {
    setBusy(true); setError("");
    try {
      if (confirmation.type === "revoke") {
        await api("/api/user/credentials", { method: "DELETE", body: { provider: confirmation.provider } });
  ...
  return <main className="account-page"><header className="page-heading"><div><h1>{f("account")}</h1><p>{session?.user?.email}</p></div><button className="icon-button" aria-label={f("refresh")} title={f("refresh")} onClick={() => { account.reload(); credentials.reload(); }}><RefreshCw size={18} /></button></header>
    {(error || account.error || credentials.error) && <p className="inline-error" role="alert">{error || f("loadError")}</p>}
    ...
    <section className="account-section"><h2>{f("subscription")}</h2>{!account.data?.subscriptions.length && <p>{f("noData")}</p>}{account.data?.subscriptions.map(sub => <div className="subscription-row" key={sub.id}><div><strong>{sub.planId}</strong><p>{f.has(sub.status) ? f(sub.status) : sub.status} · {new Date(sub.periodEnd).toLocaleDateString(locale)}</p>{sub.scheduledPlanId && <p>{f("scheduledPlan", { plan: sub.scheduledPlanId })}</p>}</div>{["active", "trialing", "past_due"].includes(sub.status) && <div className="flex flex-wrap gap-2"><button className="button compact" onClick={() => setConfirmation({ type: "subscription", id: sub.id, action: sub.cancelAtPeriodEnd ? "resume" : "cancel", key: crypto.randomUUID() })}>{f(sub.cancelAtPeriodEnd ? "resumeSubscription" : "cancelSubscription")}</button><button className="button compact" onClick={() => { setNextPlan(sub.planId === "sub_standard" ? "sub_pro" : "sub_standard"); setConfirmation({ type: "subscription", id: sub.id, action: "change", key: crypto.randomUUID() }); }}>{f("changePlan")}</button></div>}</div>)}</section>
    <section className="account-section"><h2><KeyRound size={18} />{f("credentials")}</h2><form className="credential-form" onSubmit={saveKey}><label>{f("provider")}<select value={provider} onChange={e => setProvider(e.target.value)}>{["openai", "gemini", "fashn"].map(value => <option key={value} value={value}>{value}</option>)}</select></label><label>{f("secret")}<input type="password" autoComplete="new-password" minLength={16} maxLength={4096} required value={secret} onChange={e => setSecret(e.target.value)} /></label><button className="button" disabled={busy}><Save size={16} />{f("bind")}</button></form><div className="credential-list">{credentials.data?.credentials.map(row => <div className="credential-row" key={row.id}><strong>{row.provider}</strong><code>{row.mask}</code><span>{f(row.status === "active" ? "active" : "revoked")}</span><button className="icon-button" title={f("revoke")} aria-label={`${f("revoke")} ${row.provider}`} disabled={row.status !== "active" || busy} onClick={() => setConfirmation({ type: "revoke", provider: row.provider })}><Trash2 size={16} /></button></div>)}</div></section>
    ...
    {confirmation && <Modal label={f(confirmation.type === "revoke" ? "revoke" : "subscription")} onClose={() => { if (!busy) setConfirmation(null); }}><h2>{f(confirmation.type === "revoke" ? "revoke" : "subscription")}</h2><p>{f(confirmation.type === "revoke" ? "signOutAfterRevoke" : "confirmCancel")}</p>{confirmation.action === "change" && <select aria-label={f("changePlan")} value={nextPlan} onChange={e => setNextPlan(e.target.value)}><option value="sub_standard">Standard</option><option value="sub_pro">Pro</option></select>}<div className="dialog-actions"><button className="button" disabled={busy} onClick={() => setConfirmation(null)}>{f("cancel")}</button><button className="button primary" disabled={busy} onClick={confirm}>{f("save")}</button></div></Modal>}
  </main>;
}
