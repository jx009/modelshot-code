...
function StudioContent() {
  ...
  async function uploadFiles(files, model = false) {
    ...
    if (!model && images.length + selected.length > 50) { setError(ts("batchLimit")); return; }
    ...
    try {
      for (const file of selected) {
        ...
        if (model) change("personImage", result.url); else setImages(previous => batchMode ? [...previous, result.url] : [result.url]);
      }
      setSaved(false);
  ...
  async function confirmQuote() {
    ...
    try {
      ...
      setVariants(data.tryonIds.map(id => ({ id, status: "queued", clothesImage: images[0], aspectRatio: config.aspectRatio })));
  ...
  return <div className="studio-shell" data-mobile-tab={tab}>
    ...
    <div className="studio-body"><AssetRail images={batchMode ? images : images.slice(0, 1)} batchMode={batchMode} busy={busy} uploading={uploading} onFiles={uploadFiles} onRemove={index => setImages(value => value.filter((_, i) => i !== index))} />
