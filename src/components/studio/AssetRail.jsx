...
export default function AssetRail({ images, batchMode, busy, uploading, onFiles, onRemove }) {
  ...
  return <aside className="asset-panel">
    ...
    <div className="asset-list">
      {images.map((url, index) => <div key={`${url}-${index}`} className="asset-item"><AssetImage src={url} alt={`${t("preview")} ${index + 1}`} /><div><span>{t("assets")} {String(index + 1).padStart(2, "0")}</span><small>{t("preview")}</small></div><button className="icon-button" disabled={busy} title={t("remove")} aria-label={`${t("remove")} ${index + 1}`} onClick={() => onRemove(index)}><X size={15} /></button></div>)}
